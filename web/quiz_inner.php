...
<div id="quiz-answers">
    ...
    <form class="quiz-form" action="actions/check_ans.php" method="POST">
        ...
        if($multiple_sol){ // ha több a megoldás
            ...
            while($row = $result->fetch_assoc()){
                echo '<div class="quiz-ans" id="quiz-d-'.$row['id'].'"><div class="quiz-ans-ch"><input type="checkbox" name="ans-' .$row['id'].'" value="'.$row['id'].'"></div><div class="quiz-ans-text">'.htmlspecialchars($row['valasz'], ENT_QUOTES,'UTF-8').'</div></div>';
            }
            echo '</div>';
        }else{ // ha egy megoldás van
            echo '<div class="quiz-ans-one">';
            while($row = $result->fetch_assoc()){
                echo '<div class="quiz-ans" id="quiz-d-'.$row['id'].'"><div class="quiz-ans-ch"><input type="radio" name="ans" value="'.$row['id'].'"></div><div class="quiz-ans-text">'.htmlspecialchars($row['valasz'], ENT_QUOTES,'UTF-8').'</div></div>';
            }
            echo '</div>';
        }
        ?>
    ...
    <form class="quiz-form" action="actions/next_previous.php" method="POST">
        ...
        <input id="quiz-check" type="submit" name="quiz-advance" value="Tovább">
            ...
            <?php
                if(isset($_SESSION['quiz_check'])){
                     foreach ($_SESSION['quiz_check'] as $e) echo $e;
                    echo '<script>document.querySelector(".quiz-ans").classList.add(".quiz-wrong");</script>';
                    foreach($_SESSION['quiz_check'] as $e){
                    echo '<script>
                            let element = document.getElementById("quiz-d-'.$e.'").classList.add.(".quiz-good");
    
                          </script>';
                    }
                    unset($_SESSION['quiz_check']);
                    }
            ?>
